...
class GPT(nn.Module):
  def __init__(self, model_config, init_weights=None):
    ...
    self.stacks = nn.ModuleList([DecoderStack(dim=self.dim, n_hidn=self.n_hidn, num_heads=self.num_heads, bias=self.bias, init_weights=init_weights) for _ in range(self.n_stack)])
    self.fc = nn.Linear(self.dim, self.oupt_dim, bias=self.bias)
    self.softmax = nn.Softmax(dim=-1)
  # __init__():

  def forward(self, input):
    for stack in self.stacks: input = stack(input, input)
...
if __name__ == "__main__":
  ...
  # init_weights()
  vurt = GPT(model_config, init_weights=None)
