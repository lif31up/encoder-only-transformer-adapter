...
class BERT(nn.Module):
  def __init__(self, num_heads: int, dim: int, oupt_dim: int, n_hidn: int, bias: bool = False, n_stack: int = 3,
    ...
    self.encoder = nn.ModuleList([EncoderStack(dim=self.dim, n_hidn=self.n_hidn, num_heads=self.num_heads,
                                               bias=self.bias, init_weights=init_weights) for _ in range(self.n_stack)])
